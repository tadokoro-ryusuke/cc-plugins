...
- **マジックナンバー**: `const MAX_RETRY = 3;` — ドメイン上の意味を持つ数値や繰り返し使う方針は名前付き定数にする。一度しか使わない自明なリテラルまで抽象化しない
- **設定値**: 環境変数または設定ファイル（API Key, URL, パス）
- **UI文字列**: 定数や言語ファイルで管理

## 型の厳格性（言語別）

原則: **型システムの「逃げ道」は既定で使わない。使う場合は範囲を狭め、理由をコメントで残す。**

通常の実装上の選択に、新たな承認は求めない。公開契約・セキュリティ保証・入力の境界に関わる箇所で逃げ道を使う場合は、リスクゲートのレビューに回す（→ delegation-and-review.md §5）。

既存のリポジトリでは、設定済みの strictness とチェックに従い、個別のタスクのために新しいリンタや型チェッカーを持ち込まない。以下の言語別の既定は、新しく設定するときや設定がないときに使う。
