...
- 実装者の自己申告をそのまま承認に使わない(自己レビューは自己弁護的になりやすい)。リスクゲートに該当する変更は、実装の文脈を持たない独立レビューに回す(ゲートの対象と進め方 → dev-core:best-practices の references/delegation-and-review.md)。独立性は、実装者が TDD でテストを書くことを妨げない。
