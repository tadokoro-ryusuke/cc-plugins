...
症状を説明できる、互いに独立した仮説を立てる。数は問題の規模と、症状を説明できる競合仮説の数に合わせる。有力な説明が1つしかなく競合する仮説がないなら、チームの調整コストに見合わないので、チームを作らず `debug` スキルの4フェーズで単独に調査する。仮説をユーザーに提示し、追加や修正があれば反映する。

### Phase 3: Agent Team 作成と調査開始

Lead は指示・調整・統合を担い、調査は Investigator に任せる。Lead が自分の仮説で調査を先に進めると、Investigator の結果を読む前に結論へ寄り、独立した検証の価値が薄れるため。

#### タスクリストの作成

チームメイトを spawn する前に、共有タスクリストを作成する。タスクは仮説ごとに作り、数と粒度は仮説の検証範囲に合わせる。
...
仮説ごとに 1 人の Investigator を spawn する。プロジェクトコンテキスト（CLAUDE.md、MCP サーバー、スキル等）は自動で読み込まれるため、spawn プロンプトには問題の情報と担当仮説だけを簡潔に渡す。
...
- **有力候補あり**: 複数の Investigator が同じ仮説を支持し、有効な反証が残っていない → 有力な原因として提示し追加検証を提案
