...
$ARGUMENTS に基づいて Agent Team を作成し、3 つの専門的な視点（要件分析・設計・批評）で議論・検証した上で TDD 計画を立案する。
...
```
シニアエンジニアとして「$ARGUMENTS」の要件と設計を批評してください。
...
Task ツールで `dev-core:issue-creator` エージェントを呼び出し、計画書から GitHub Issue を作成する。
